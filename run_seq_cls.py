...
def evaluate(args, model, eval_dataset, mode, epoch=None):
    ...
    with open(output_eval_file, "w") as f_w:
        ...
        for key in sorted(results.keys()):
            ...
            f_w.write(f'real_label: {out_label_ids}     predict_label: {preds}\n\n')
...
def main(cli_args):
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            if test_dataset != None:
                result_test = evaluate(args, model, test_dataset, mode="test", epoch=epoch)
                result_test = dict((k + "_{}".format(epoch), v) for k, v in result_test.items())
                results_test.update(result_test)

                output_eval_file = os.path.join(args.output_dir, "eval_results.txt")
                with open(output_eval_file, "w") as f_w:
                    for key in sorted(results_test.keys()):
                        f_w.write("{} = {}\n".format(key, str(results_test[key])))

            if dev_dataset != None:
                result_dev = evaluate(args, model, dev_dataset, mode="dev", epoch=epoch)
                result_dev = dict((k + "_{}".format(epoch), v) for k, v in result_dev.items())
                results_dev.update(result_dev)

                output_dev_file = os.path.join(args.output_dir, "dev_results.txt")
                with open(output_dev_file, "w") as f_w:
                    for key in sorted(results_dev.keys()):
                        f_w.write("{} = {}\n".format(key, str(results_dev[key])))
