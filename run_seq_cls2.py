...
def train(args,
    ...
    for epoch in mb:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    if args.evaluate_test_during_training:
                        ...
                        evaluate(args, model, train_dataset, "train", global_step)
        ...
        '''
        if args.evaluate_test_during_training:
            evaluate(args, model, train_dataset, "train", epoch + 1)
...
def main(cli_args):
    ...
    special_tokens_dict = {'additional_special_tokens': ['[SPK_A]', '[SPK_B]', '[SPK_C]', '[SPK_D]', '[sum]']}
    ...
    tokenizer.save_pretrained(f"{tokenizer_output_dir}")
    ...
    args.save_steps = (len(train_dataset) / args.train_batch_size) + 1
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            result2 = evaluate(args, model, dev_dataset, mode="dev", epoch=epoch)
            #result3 = evaluate(args, model, train_dataset, mode="train", epoch=epoch)
            result = dict((k + "_{}".format(epoch), v) for k, v in result.items())
            result2 = dict((k + "_{}".format(epoch), v) for k, v in result2.items())
            #result3 = dict((k + "_{}".format(epoch), v) for k, v in result3.items())
            results.update(result)
        ...
        output_dev_file = os.path.join(args.output_dir, "eval_Dev_results.txt")
        with open(output_dev_file, "w") as f_w:
            for key in sorted(results2.keys()):
                f_w.write("{} = {}\n".format(key, str(results2[key])))

        output_train_file = os.path.join(args.output_dir, "eval_train_results.txt")
        with open(output_train_file, "w") as f_w:
            for key in sorted(results3.keys()):
                f_w.write("{} = {}\n".format(key, str(results3[key])))
